    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.workers.update", [$worker->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fio') ? 'is-invalid' : '' }}" type="text" name="fio" id="fio" value="{{ old('fio', $worker->fio) }}">
                @if($errors->has('fio'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('work') ? 'is-invalid' : '' }}" name="work_id" id="work_id">
                    @foreach($works as $id => $entry)
                        <option value="{{ $id }}" {{ (old('work_id') ? old('work_id') : $worker->work->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('work'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{{ old('note', $worker->note) }}</textarea>
                @if($errors->has('note'))
                    <div class="invalid-feedback">
                        {{ $errors->first('note') }}
                    </div>
